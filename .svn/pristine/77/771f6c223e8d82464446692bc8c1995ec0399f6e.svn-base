...
public class PageTool {
    ...
    public String getPageString(String url, Page page) {
        if (url.indexOf("?") != -1) {
            this.url = "<a href='" + url + "&pageNumber=";
        } else {
            this.url = "<a href='" + url + "?pageNumber=";
        }
        this.page = page;
        ...
        return "" +
                " " + getLeftTools() + getCenter() + getRightTools() + "";
    }
    ...
    public String getPageStringForJsSeek(String url, Page page) {
        this.page = page;
        this.currentPage = page.getCurrentPage();
        this.pageSize = page.getPageSize();
        this.totalPageCount = page.getTotalPageCount();
    ...
    private String getLeftTools2Seek() {
        ...
        if (currentPage == 1) {
            ...
            leftString = new StringBuffer("<a href='javascript:;' onclick='seekExperts(1);'><font class='page_up_down'>« 首页</font></a><a href='javascript:;' onclick='seekExperts(" + (getCurrentPage() - 1) + ");'><font class='page_up_down'>上一页</font></a>");
        }
        return leftString.toString();
    }
    ...
    private String getRightTools2Seek() {
        ...
        if (currentPage == totalPageCount || totalPageCount == 0) {
            ...
            rightString = new StringBuffer("<a href='javascript:;' onclick='seekExperts(" + (getCurrentPage() + 1) + ");'><font class='page_up_down'>下一页</font></a><a href='javascript:;' onclick='seekExperts(" + (getTotalPageCount()) + ");'><font class='page_up_down'>尾页 »</font></a>");
        }
        return rightString.toString();
    }
        ...
        if (totalPageCount <= lineCursor) {
            for (int i = 1; i < this.totalPageCount + 1; i++) {
                if (i == this.currentPage) {
                    ...
                    centerString.append("<a href='javascript:;' onclick='seekExperts(" + i + ");'>" + i + "</a>");
                }
            }
        } else {
            int s = currentPage - cursor;
